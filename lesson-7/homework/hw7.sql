...
select customerID, sum(SaleAmount) as total_sale_per_customer
from sales
group by CustomerID
...
select departmentNAME, count(employeeID) as emp_IN_DEP 
from Employees
group by departmentname
having count(employeeID)>5

--11. Write a query to calculate the total sales and average sales for each product category from the Sales table.
select productID, sum(saleamount) as total_saleamount, avg(saleamount) as avg_saleamount
from sales
group by productID

--12.Write a query to count the number of employees from the Department HR.
select departmentname,  count(EmployeeID) as numb_of_employees_in_hr
from Employees
...
select departmentname, min(salary) as min_salary,
...
select departmentname, avg(salary) as avg_salary, 
count(employeeID) as employees_in_dep
from Employees
group by departmentname
...
select customerID, sum(SaleAmount) as sale_amount
from sales
group by CustomerID
...
--23.Write a query to find total amount for the orders which weights more than $50 for each customer along with their least purchases.(least amount might be lower than 50, 
...
select year(SaleDate) as order_year,
