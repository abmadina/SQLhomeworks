...
join Enrollments as e on s.StudentID=e.CourseID
join Courses as c on c.CourseID=e.EnrollmentID
...
join Departments as d
on e.DepartmentID=d.DepartmentID
where d.DepartmentName like 'human resources'
...
select count(EmployeeID) as employee_in_dep, d.DepartmentName from Employees as e
join Departments as d
on e.DepartmentID=d.DepartmentID
...
join Sales as s on s.SaleDate =o.OrderDate
on p.=s.
--12. **Using the `Customers` and `Orders` tables**, write a query to return customer names who have placed at least one order.  
--   🔁 *Expected Columns:* `FirstName`, `LastName`, `TotalOrders`
select c.firstname, c.LastName, count(o.orderID), c.CustomerID  from Customers as c
...
join Customers as c
on c.CustomerID=o.CustomerID
where year(o.OrderDate) like '2022'
...
e2.name as Employee2, e1.ManagerID, e1.Salary from Employees as e1
join Employees as e2 
on e1.EmployeeID=e2.ManagerID
...
select sl.saleID, p.ProductName, sl.saleamount from sales as sl
join Products as p
on p.ProductID=sl.productID
where sl.saleamount>500
...
where c.CourseName is not 'math101'
