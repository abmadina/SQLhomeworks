...
select o.OrderID, c.FirstName + ' ' + c.LastName as Customer_name, OrderDate from orders as o
join customers as c
...
select d.DepartmentName, max(salary) as max_salary from Departments as d
join Employees as e
on e.DepartmentID=d.DepartmentID
...
select c.FirstName+' '+c.LastName, OrderID, OrderDate from customers as c
join Orders as o
...
--   **Tables Used**: `Orders` , `Customers`
select c.FirstName+' '+c.LastName as Customers_name, count(OrderID) from customers as c
join Orders as o
on c.CustomerID=o.CustomerID
...
--   **Tables Used**: `Customers`, `Orders`
select c.FirstName+' '+c.LastName as customer_name, max(orderdate) from Customers as c
join Orders as o
on c.CustomerID=o.CustomerID
group by c.FirstName+' '+c.LastName 
...
--   **Tables Used**: `Orders`, `Customers`
select c.FirstName+' '+c.LastName as customer_name, sum(o.TotalAmount) from Customers as c
join Orders as o
on o.CustomerID=c.CustomerID
group by c.FirstName+' '+c.LastName 
...
select p.ProductName , count (p.productname) as TotalSalesAmount from Products as p
join sales as s
on p.ProductID=s.ProductID
...
join sales as s
on p.ProductID=s.ProductID
where year(SaleDate)=2023 or StockQuantity>100
...
join Departments as d
on d.DepartmentID=e.DepartmentID
where year(hiredate)>2020 or DepartmentName='sales'
...
join Orders as o
on o.CustomerID=c.CustomerID
where c.Country='usa' or c.Address like '[0-9],[0-9],[0-9],[0-9]%'
...
join Sales as s
on p.ProductID=s.ProductID
where p.Category = 'Electronics' and SaleAmount>350
--16. **Return**: `CategoryName`, `ProductCount`  
--    **Task**: Show the number of products available in each category.  
--    **Tables Used**: `Products`, `Categories`
select c.CategoryName, count(p.productname) from Categories as c
join Products as p
...
join orders as o
on c.City='Los Angeles' and TotalAmount>300
...
join Departments as d
on e.DepartmentID=d.DepartmentID
where DepartmentName in ('Sales','Marketing') or salary>60000
---
```sql
